...
- **Right comparison panel coloring is `:nth-child(3)`.** Because `.primary-text-content.comparison-mode` lays out as `[.comparison-side, .comparison-vs, .comparison-side]`, the right side is child 3. `.comparison-side:nth-child(3) .word-group.highlighted` is how the blue (`--color-highlight-2`) gets applied. Fragile: rearrange the DOM and the right-side color silently breaks. Word groups with `"side": "right"` in the data don't drive coloring — DOM position does.
