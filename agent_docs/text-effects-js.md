...
- `wrapWords(container, words)` — for each `(groupId, group)` pair in the data file's `words` map, find `group.he` inside every `.primary-he` descendant and `group.en` inside every `.primary-en`, wrapping matches in `<span class="word-group" data-word="<groupId>" data-lang="he|en">`.
- `_wrapPhrase(el, phrase, groupId, lang)` — the regex-based wrapper. Escapes special regex chars, then collapses internal whitespace to the class `[\s​ ־-]*` so the match tolerates regular spaces, zero-width space, non-breaking space, Hebrew maqaf (U+05BE), and ASCII hyphen.
...
- `crossfadeTo(sectionEl, newVerseData)` — finds the pre-rendered `.primary-text-content` whose `data-ref` matches `newVerseData.ref`, swaps `.active` from the current element to it, and adds a transient `.fading-out` class (cleared after 600 ms). If `newVerseData.words` is set, re-wraps words on the new element.

## Non-obvious patterns

- **Wrapping is idempotency-fragile.** `_wrapPhrase` reads and re-writes `el.innerHTML`. If the phrase has already been wrapped, the phrase text *still appears* inside the existing `<span class="word-group">…</span>`, so the regex re-matches and produces nested spans (`<span data-word="x"><span data-word="x">phrase</span></span>`). The active flow can trigger this: `buildVerseContent` schedules an initial wrap via `requestAnimationFrame`, `crossfadeTo` wraps again on the target verse, and `engine.js` re-wraps a third time 100 ms later. Visible behavior is fine — CSS still matches `[data-word="..."]` — but the DOM grows nested spans over time. If you're touching this, consider a "skip if already wrapped" check on the container.
...
- **`setupComparison` is dead code.** Defined at the bottom of the module but never called from `engine.js`. The `comparison-mode` class is added by `buildComparisonVerse` directly.
- **`currentVerse` is unused.** Declared on the object but never read or written. Safe to remove.
